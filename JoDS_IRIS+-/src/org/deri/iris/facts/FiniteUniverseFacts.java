...
/**
 * Definition: Unsafe negation A rule has a variable in a negated sub-goal that
 * also occurs in the head, but nowhere else.
 * 
 * Definition: Unsafe A rule has a variable that does not occur in a positive
 * ordinary literal.
 * 
 * Create a $UNIVERSE$(?X) relation.
 * 
 * To handle unsafe negation (Gelder, Well-founded semantics, p.24): 1. For
 * every rule, extract every occurrence of a non-ground-term, e.g. ?X or f(?Y)
 * 2. Substitute a distinct ground term (not found anywhere else) for the
 * variable(s), e.g. '$X', f('$Y') 3. Add this term to $UNIVERSE$
 * 
 * To handle all unsafe rules (with free variables in head or built-ins): 5. Add
 * all concrete terms that occur anywhere in the program (facts and rules) to
 * $UNIVERSE$
 * 
 * For both: 6. Add $UNIVERSE$(?variable) sub-goals for each free variable in
 * each rule
 */
public class FiniteUniverseFacts implements IFacts {
     ...
     * 
     * @param facts
     *            The starting facts of the knowledge-base.
     * @param rules
	...
	if (facts == null)
	    throw new IllegalArgumentException(
		    "Argument 'facts' must not be null.");

	if (rules == null)
	    throw new IllegalArgumentException(
		    "Argument 'rules' must not be null.");

	mFacts = facts;

	// mUniverse = mFacts.get( UNIVERSE );
	mUniverse = new RelationFactory().createRelation();
    ...
    /**
     * Extract ground terms from rules and create unique values for each
     * variable found.
     * 
     * @param rules
     *            The rules of the knowledge-base.
	...
	if (term instanceof IVariable) {
	    ...
	    addToUniverse(Factory.TERM.createString(variable.getValue()
	 ...
	 * 
	 * @param child
	 *            The wrapped underlying facts object.
	 */
	...
	public boolean add(ITuple tuple) {
	    ...
	    /*
	     *  If this is a tuple not seen before and contains no fresh terms
	     *  then it might have new terms in it. 
	     */
	    if (!result && tuple.getFreshTerms().size() == 0)
		addToUniverse(tuple);
	...
	if (predicate == null)
	    throw new IllegalArgumentException(
		    "Argument 'predicate' must not be null.");
     ...
     * 
     * @param tuple
     *            The tuple whose terms are to be added.
     */
    ...
    /**
     * Add a term (recursively in the case of constructed terms) to the universe
     * relation.
     * 
     * @param term
     *            The term to add.
	...
	// NOTE
	// If term = f(g(h(1))) then 1, h(1), g(h(1)) and f(g(h(1))) will get
