...
/**
 * IRIS duration objects are implemented using the javax.xml.datatype.Duration
 * class. However, there are a number of problems/bugs in the implementation of
 * this class, specifically with the add(), subtract(), equals() and compareTo()
 * methods. These methods do not work correctly when created using BigDecimal
 * seconds, even though the seconds component may be a whole number of seconds.
 */
public class XmlDurationWorkAroundHelper {
    ...
    /**
     * Subtract one Duration from another, avoiding the runtime library bug that
     * gives incorrect results when using decimal seconds.
     * 
     * @param d1
     *            The first duration
     * @param d2
	...
	if (sign1 && sign2) {
	    ...
	    if (comparison >= 0)
		return subtractSmallerPositiveDurationFromLargerPositiveDuration(
			d1, d2);
	    else
		return subtractSmallerPositiveDurationFromLargerPositiveDuration(
			d2, d1).negate();
    ...
    /**
     * Special equality method that gets around the problem in java 1.5, where
     * years and months are converted to days after arithmetic.
     * 
     * @param d1
     *            First duration
     * @param d2
     *            Second duration
     * @return true if the two duration objects are equal in either raw or
     *         day-normalised form.
     */
    ...
    /**
     * Special compare method that gets around the problem in java 1.5, where
     * years and months are converted to days after arithmetic.
     * 
     * @param d1
     *            First duration
    ...
    private static int[] forwardFields = { Calendar.YEAR, Calendar.MONTH,
	    Calendar.DAY_OF_MONTH, Calendar.HOUR_OF_DAY, Calendar.MINUTE,
	    Calendar.SECOND, Calendar.MILLISECOND };

    private static int[] reverseFields = { Calendar.MILLISECOND,
	...
	return FACTORY.newDuration(positive, BigInteger.valueOf(year),
		BigInteger.valueOf(g1.get(Calendar.MONTH)), BigInteger
			.valueOf(g1.get(Calendar.DAY_OF_MONTH) - 1), BigInteger
			.valueOf(g1.get(Calendar.HOUR_OF_DAY)), BigInteger
			.valueOf(g1.get(Calendar.MINUTE)), seconds);
    }

    private static String toString(GregorianCalendar g1) {
	StringBuilder buffer = new StringBuilder();

	buffer.append(g1.get(Calendar.YEAR) + "/"
		+ (g1.get(Calendar.MONTH) + 1) + "/"
		+ g1.get(Calendar.DAY_OF_MONTH));
	buffer.append("  ");
	buffer.append(g1.get(Calendar.HOUR_OF_DAY) + ":"
		+ g1.get(Calendar.MINUTE) + ":" + g1.get(Calendar.SECOND) + "."
		+ g1.get(Calendar.MILLISECOND));

	return buffer.toString();
    }

    private static void subtractField(GregorianCalendar g1,
	    GregorianCalendar g2, int field) {
    ...
    /**
     * Test for mathematical equality of two BigDecimal objects (not
    ...
    /**
     * Return just the fractional part of the seconds component of a duration
     * object, i.e. 1:2:34.45 => 0.45
     * 
     * @param duration
     *            The duration object to examine.
     * @return The fractional part of the seconds field.
     */
    private static BigDecimal fractionalSeconds(Duration duration) {
	BigDecimal seconds = (BigDecimal) duration
	...
	return FACTORY.newDuration(positive, year, month, day, hour, minute,
		second);
    }
	...
	BigDecimal properSeconds = BigDecimal.valueOf(stripResult.getSeconds())
		.add(extraSeconds);

	return FACTORY.newDuration(true, BigInteger.valueOf(stripResult
		.getYears()), BigInteger.valueOf(stripResult.getMonths()),
		BigInteger.valueOf(stripResult.getDays()), BigInteger
			.valueOf(stripResult.getHours()), BigInteger
			.valueOf(stripResult.getMinutes()), properSeconds);
    }
     ...
     * 
     * @param d1
     *            The larger positive duration
     * @param d2
     *            The smaller positive duration
     * @return The difference
     */
    private static Duration subtractSmallerPositiveDurationFromLargerPositiveDuration(
    ...
    /**
     * Java runtime 1.5 is inconsistent with its handling of days in Duration
     * objects.
     * 
     * @param duration
     *            A duration object to be normalised
     * @return A day-normalised duration, i.e. all years and months converted to
     *         days, e.g. 1Y 3M 3D => 458 days
     */
    private static javax.xml.datatype.Duration normaliseDays(
	    javax.xml.datatype.Duration duration) {
	final long DAYS_PER_MONTH = 30;
	final long DAYS_PER_YEAR = 365;

	BigInteger days = (BigInteger) duration
		.getField(DatatypeConstants.DAYS);
	BigInteger months = (BigInteger) duration
		.getField(DatatypeConstants.MONTHS);
	BigInteger years = (BigInteger) duration
		.getField(DatatypeConstants.YEARS);

	BigInteger normalisedDays = years.multiply(BigInteger
		.valueOf(DAYS_PER_YEAR));
	normalisedDays = normalisedDays.add(months.multiply(BigInteger
		.valueOf(DAYS_PER_MONTH)));
	normalisedDays = normalisedDays.add(days);

	BigInteger hours = (BigInteger) duration
		.getField(DatatypeConstants.HOURS);
	BigInteger minutes = (BigInteger) duration
	...
	return FACTORY.newDuration(positive, BigInteger.ZERO, BigInteger.ZERO,
		normalisedDays, hours, minutes, seconds);
    }

    private static javax.xml.datatype.Duration normaliseSeconds(
	    javax.xml.datatype.Duration duration) {
	BigInteger years = (BigInteger) duration
		.getField(DatatypeConstants.YEARS);
	BigInteger months = (BigInteger) duration
	...
	return FACTORY.newDuration(positive, years, months, days, hours,
		minutes, seconds);
    }
    ...
    private static final Duration DURATION_1_SECOND = FACTORY.newDuration(true,
	    0, 0, 0, 0, 0, 1);
}
