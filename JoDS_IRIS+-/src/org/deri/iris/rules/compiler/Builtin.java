...
public class Builtin extends RuleElement {
     ...
     *             unbound variables.
     */
    public Builtin(List<IVariable> inputVariables, IBuiltinAtom builtinAtom,
	    boolean positive, Configuration configuration)
	    throws EvaluationException {
	...
	// TODO Properly calculate output variables and indices for negative
	// literals
	...
	for (int t = 0; t < builtinTuple.size(); ++t) {
		...
		if (indexFromInput == -1) {
		    ...
		    indicesFromBuiltOutputTupleToCopyToOutputRelation
	...
	if (uniqueUnboundBuiltInVariables.size() > mBuiltinAtom
		.maxUnknownVariables())
	    throw new EvaluationException(
		    "Too many unbound variables for built-in '" + mBuiltinAtom
			    + "' unbound variables: " + unboundBuiltInVariables);

	// The indexes of terms in the built-in output tuple to copy to the
	// output relation
	mIndicesFromBuiltInOutputTupleToCopyToOutputRelation = Utils
		.integerListToArray(indicesFromBuiltOutputTupleToCopyToOutputRelation);
	...
	for (int i = 0; i < leftRelation.size(); ++i) {
		...
		terms[t] = index == -1 ? mBuiltinAtom.getTuple().get(t) : input
			.get(index);
	    }

	    ITuple builtinInputTuple = Factory.BASIC.createTuple(terms);
	    ITuple builtinOutputTuple = mBuiltinAtom
		    .evaluate(builtinInputTuple);
	...
	if (builtinOutputTuple.size() == 0)
	    return inputTuple == null ? Factory.BASIC.createTuple()
		    : inputTuple;

	ITerm[] terms = new ITerm[(inputTuple == null ? 0 : inputTuple.size())
		+ mIndicesFromBuiltInOutputTupleToCopyToOutputRelation.length];
