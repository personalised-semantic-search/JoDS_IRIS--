...
/**
 * A compiled rule element representing a built-in predicate with constructed
 * terms arguments.
 */
public class BuiltinForConstructedTermArguments extends RuleElement {
    ...
    public BuiltinForConstructedTermArguments(List<IVariable> inputVariables,
	    IBuiltinAtom builtinAtom, boolean positive,
	...
	if (mBuiltinAtom instanceof EqualBuiltin
	    ...
	    throw new EvaluationException(
		    "Only equality, inequality and assignment built-in predicates can "
			    + "have constructed terms as arguments. The problem atom is: "
			    + builtinAtom);

	// So at this point, we know that we have a built-in that can use
	// constructed terms as arguments.
	// Next, we have to classify the arguments as either:
	// a) grounded or groundable (so need indices from input relation for
	// each variable)
	// b) a simple variable term
	// c) something else => can't handle, e.g. a constructed term with
	// unbound variables
	...
	List<IVariable> variablesInBuiltinTuple = TermMatchingAndSubstitution
		.getVariables(builtinTuple, false);
	List<IVariable> unboundVariables = new ArrayList<IVariable>(
		variablesInBuiltinTuple);
	unboundVariables.removeAll(inputVariables);

	if (mType == TYPE.INEQUALITY) {
	    if (unboundVariables.size() > 0)
		throw new EvaluationException(
			"Not enough grounded variables for in-equality with constructed terms. The problem atom is: "
				+ builtinAtom);
	}
	...
	mIndicesOfBuiltinVariablesFromInputRelation = Utils
		.integerListToArray(indicesOfBuiltinVariablesFromInputRelation);
	...
	if (mType == TYPE.UNIFICATION) {
	    List<IVariable> uniqueVariablesInBuiltinTuple = TermMatchingAndSubstitution
		    .getVariables(builtinTuple, true);
	    mUniqueUnboundVariables = new ArrayList<IVariable>(
		    uniqueVariablesInBuiltinTuple);
		...
		if (mUniqueUnboundVariables.size() > 0)
		    throw new RuleUnsafeException(
			    "Negated unify is not safe when some variables are unbound. The problem atom is: "
				    + builtinAtom);
	    }
	} else
	    mUniqueUnboundVariables = new ArrayList<IVariable>();
	...
	for (int i = 0; i < leftRelation.size(); ++i) {
	    ...
	    ITuple builtinInputTuple = TermMatchingAndSubstitution
		    .substituteVariablesInToTuple(mBuiltinAtom,
			    inputTuple,
			    mIndicesOfBuiltinVariablesFromInputRelation);
		...
		boolean unified = TermMatchingAndSubstitution.unify(t1, t2,
			variableMap);

		if (mPositive) {
		    if (unified) {
			ITerm[] terms = new ITerm[inputTuple.size()
				+ mUniqueUnboundVariables.size()];
