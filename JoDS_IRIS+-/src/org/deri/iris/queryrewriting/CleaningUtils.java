	...
	public static IRule cleanPrefix(final IRule r, final String prefix) {
		...
		for (final ILiteral l : r.getBody()) {
			...
			if (p.getPredicateSymbol().startsWith(prefix)) {
				body.add(bf.createLiteral(l.isPositive(), bf.createPredicate(
				        p.getPredicateSymbol().replace(prefix, ""), p.getArity()), l.getAtom().getTuple()));
			} else {
				body.add(l);
			}
		}

		return (RenamingUtils.canonicalRenaming(bf.createRule(r.getHead(), body), "X"));
	}

	public static void cleanRewriting(final QueryGraph queryGraph, final String[] auxPreds) {

		for (final IRule q : queryGraph.getRules()) {
			boolean done = false;
