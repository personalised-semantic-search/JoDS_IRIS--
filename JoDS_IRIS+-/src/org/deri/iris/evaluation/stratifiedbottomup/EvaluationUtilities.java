...
public class EvaluationUtilities {
     ...
     * @param rules
     *            The input rules.
     * @return A collection of collections of rules arranged in to strata. The
     *         rules in each stratum might be different from the imput rules.
     * @throws ProgramNotStratifiedException
     *             If none of the stratification algorithms successfully
	...
	throw new ProgramNotStratifiedException(
		"The input program is not stratified");
    }

    /**
     * Attempt to re-order the rules of a stratum such that the dependent rules
     * are evaluated first.
     * 
     * @param rules
     *            The strata of rules to re-order.
    ...
    /**
     * Optimise a collection of rules by applying each of the rule optimisations
     * to each rule in turn.
     * 
     * @param rules
     *            A collection of rules to optimise.
    ...
    /**
     * Check the given rules for rule-safety (unbound variables). In the future,
     * this method will be allowed to modify the input rules.
     * 
	...
	return (mConfiguration.ruleSafetyProcessor.process(rules));
    }
