...
/**
 * Program doubler for well-founded semantics evaluation strategy. This class
 * generates the rule-sets and facts for the alternating fixed point algorithm,
 * which is used to compute a well-founded model. The rules and facts for the
 * original logic program are used to instantiate the ProgramDoubler. After
 * which, the extractXXX and getXXX methods are used to retrieve the program
 * fragments used during the alternating fixed point loop.
 */
public class ProgramDoubler {
    /**
     * The suffix used to name the 'primed' predicates, i.e. those predicate
     * that represent definitely false facts of their unprimed counterparts.
     */
     ...
     */
    public ProgramDoubler(List<IRule> rules, IFacts facts,
    ...
    /**
     * Extract all the facts associated with unprimed predicates from the facts
     * provided.
     * 
     * @param mixed
     *            The input collection of facts tha can contain a mixture of
     *            primed and unprimed predicates.
	...
	for (IPredicate predicate : mixed.getPredicates()) {
	    if (!predicate.getPredicateSymbol().endsWith(
    ...
    /**
     * Extract all the facts associated with primed predicates from the facts
     * provided.
     * 
     * @param mixed
     *            The input collection of facts that can contain a mixture of
     *            primed and unprimed predicates.
     ...
     * 
     * @return The original positive facts, wrapped such that modifications to
     *         the returned collection does not modify the original facts.
     */
    public IFacts getPositiveStartingFacts() {
	return new OriginalFactsPreservingFacts(mOriginalFacts,
     ...
     * 
     * @return The original positive facts, wrapped such that they are
     *         associated with primed predicates and such that modifications to
     *         the returned collection does not modify the original facts.
     */
    public IFacts getNegativeStartingFacts() {
	return new InvertingFacts(new OriginalFactsPreservingFacts(
		mOriginalFacts, mConfiguration.relationFactory),
		mConfiguration.relationFactory);
    }
     ...
     * 
     * @return The starting rules. These contain only those original rules with
     *         no negative body literals.
     */
     ...
     * 
     * @return The entire rule set modified such that each rule head is primed
     *         and each positive body literal is primed.
     */
     ...
     * 
     * @return The entire rule set modified such that each negated body literal
     *         is primed.
     */
    ...
    /**
     * Extract from the original rules all those that have no negative body
     * literals.
     */
    ...
    /**
     * Compute rules such that each head is primed and each positive body
     * literal is primed. Negative body literals remain unprimed.
     */
    ...
    /**
     * Compute rules such that each negative body literal is primed. Head and
     * positive body literals remain unprimed.
     */
	...
	for (IRule rule : mOriginalRules) {
	    ...
	    mPositiveRules.add(Factory.BASIC
		    .createRule(rule.getHead(), newBody));
	}
    }
	...
	return Factory.BASIC.createLiteral(literal.isPositive(),
		makePrimedPredicate(predicate), tuple);
    }
     ...
     * 
     * @param predicate
     *            The unprimed predicate.
     * @return The primed predicate.
     */
    private static IPredicate makePrimedPredicate(IPredicate predicate) {
	String newPredicateSymbol = predicate.getPredicateSymbol()
		+ NEGATED_PREDICATE_SUFFIX;
	return Factory.BASIC.createPredicate(newPredicateSymbol, predicate
		.getArity());
    }
	 ...
	 * 
	 * @param positiveFacts
	 *            The unprimed facts to make primed.
	 * @param relationFactory
	 *            The factory used to create new relations.
	 */
	InvertingFacts(IFacts positiveFacts, IRelationFactory relationFactory) {
		...
		mPredicateRelationMap.put(makePrimedPredicate(predicate),
			positiveFacts.get(predicate));
	    }
	}
    }
