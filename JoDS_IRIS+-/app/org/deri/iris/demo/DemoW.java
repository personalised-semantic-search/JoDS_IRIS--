...
public class DemoW {
	...
	private void setup() {
	    ...
	    mProgram
		    .setText("p(succ(?X), ?Y) :- p(?X,?Z), ?Z+1=?Y, ?Y < 7.\r\n"
			    + "p(?X,?Y) :- ?X=1, ?Y=1.\r\n" + "?-p(?X,?Y).\r\n");

	    mProgram.setText("p('a'):-.\r\n" + "p(?X) :- r(?X), not s(?Y).\r\n"
		    + "?- p(?X).");

	    mProgram.setText("p(?X) :- a(?X), d(?X, ?Y)." + NEW_LINE
		    + "d(?X, ?Y) :- not s(?X, ?Y), UNIVERSE(?X), UNIVERSE(?Y)."
		    + NEW_LINE + "s(?U, ?U) :- UNIVERSE(?U)." + NEW_LINE
		    + "a(1)." + NEW_LINE + NEW_LINE + "?- p(?X)." + NEW_LINE
		    + NEW_LINE + "UNIVERSE(1)." + NEW_LINE + "UNIVERSE(2)."
		    + NEW_LINE);

	    mProgram.setText("p(?x, ?y) :- not q(?x)." + NEW_LINE
		    + "?- p(?x, ?y).");

	    mProgram.setText("p(?x) :- t(?x, ?y, ?z), not p(?y), not p(?z)."
		    + NEW_LINE + "p('b') :- not r('a')." + NEW_LINE
		    + "t( 'a', 'a', 'b')." + NEW_LINE + "t( 'a', 'b', 'a')."
		    + NEW_LINE + "?- p(?x).");

	    mProgram
		    .setText("p(0,0)."
			    + NEW_LINE
			    + "p( ?n1, succ( ?x ) ) :- p(?n, ?x), ?n + 1 = ?n1, ?n1 < 10."
			    + NEW_LINE + "//?- p(?n, ?x )." + NEW_LINE +

			    "even( succ( ?x ) ) :- ! even( ?x )." + NEW_LINE
			    + "even( 0 )." + NEW_LINE + "?- even( ?x ).");
	    ...
	    mProgram.setText("p(?x) :- q(?x), ! r(?x)." + NEW_LINE
		    + "q(?x) :- ! p(?x)." + NEW_LINE + "r('a')." + NEW_LINE
		    + "?- r(?x).");

	    mProgram.setText("p(?X,?Y) :- b(?X,?Y)." + NEW_LINE
		    ...
		    "b(1,2)." + NEW_LINE + "b(2,1)." + NEW_LINE + "g(2,3)."
		    + NEW_LINE + "g(3,2)." + NEW_LINE + "?- a(2,?Y).");

	    mProgram
		    .setText("triple(0,0,0,1)."
			    + NEW_LINE
			    + "triple(?n, ?x, ?y, ?z) :- triple(?n1, ?x1, ?y1, ?z1), ?n1 + 1 = ?n, ?n/100=?x, ?n%100=?y2, ?y2/10=?y, ?n%10=?zz, ?zz+1=?z, ?n < 1000."
			    + NEW_LINE
			    + NEW_LINE
			    + "// get all those triples where a % n = b % n (definition of congruent)"
			    + NEW_LINE
			    + "congruent( ?a, ?b, ?n ) :- triple(?k, ?a, ?b, ?n ), ?a % ?n = ?amodn, ?b % ?n = ?bmodn, ?amodn = ?bmodn."
			    + NEW_LINE
			    + NEW_LINE
			    + "// Proove that if a1 congruent a2 mod n and b1 congruent b2 mod n, then a1b1 congruent a2b2 mod n"
			    + NEW_LINE
			    + "mul( ?a1b1, ?a2b2, ?n ) :- congruent( ?a1, ?a2, ?n ), congruent( ?b1, ?b2, ?n ), ?a1*?b1=?a1b1, ?a2*?b2=?a2b2."
			    + NEW_LINE
			    + NEW_LINE
			    + "// The multiplied triples where the congruency rule does not hold."
			    + NEW_LINE
			    + "exceptions_to_rule( ?x,?y,?n ) :- mul( ?x,?y,?n), ?x % ?n = ?xmodn, ?y % ?n = ?ymodn, ?xmodn != ?ymodn."
			    + NEW_LINE
			    + NEW_LINE
			    + "// Ths should be empty if the congruency rule is correct and the reasoner behaves correctly."
			    + NEW_LINE
			    + "?-exceptions_to_rule( ?a1b1,?a2b2,?n )."
			    + NEW_LINE);

	    // Unsafe rule example
	    mProgram.setText("p( ?x ) :- a( ?x ), diff( ?x, ?y )." + NEW_LINE
		    + "diff( ?x, ?y ) :- not same( ?x, ?y )." + NEW_LINE
		    + "same( ?x, ?x ) :- ." + NEW_LINE + "a(1)." + NEW_LINE +

		    "?- p(?x).");

	    // Unstratified example.
	    mProgram
		    .setText("republican(?x) :- like_guns(?x), not hippy(?x)."
			    + NEW_LINE
			    + "democrat(?x) :- like_hippies(?x), not republican(?x)."
			    + NEW_LINE
			    + "hippy(?x) :- like_flowers(?x), not republican(?x), not democrat(?x)."
			    + NEW_LINE + NEW_LINE + "like_flowers('a')."
			    + NEW_LINE + "like_hippies('b')." + NEW_LINE
			    + "like_guns('c')." + NEW_LINE + NEW_LINE
			    + "?- republican(?x)." + NEW_LINE
			    + "?- democrat(?x)." + NEW_LINE + "?- hippy(?x).");

	    mProgram
		    .setText("some_calc(?X,?Z) :- MULTIPLY(?X,?X,?Y), ADD(?Y,-1, ?Z). "
			    + NEW_LINE + "?-some_calc(5,?Z).");

	    mProgram
		    .setText("p( _datetime( 1999, 12, 31, 23, 59, 59 ), _datetime( 1998, 11, 30, 22, 58, 58 ) )."
			    + NEW_LINE
			    + "p( _datetime( 2000, 1, 1, 0, 0, 0 ), _datetime( 1999, 12, 31, 23, 59, 0 ) )."
			    + NEW_LINE + NEW_LINE + "?- p(?x,?y), ?x-?y=?z.");
