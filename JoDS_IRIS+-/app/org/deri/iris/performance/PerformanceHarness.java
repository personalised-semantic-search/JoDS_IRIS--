...
public class PerformanceHarness {
	...
	output.append("On machine: ").append(localHost.getHostName()).append(
		" (").append(localHost).append(")").append(NEW_LINE);

	if (showPrograms) {
	    for (int p = 0; p < mPrograms.size(); ++p) {
		pause(1);
		output.append(THIN_LINE);
		output.append("Program ").append(p).append(" is:").append(
			NEW_LINE);
	...
	pause1();
	for (int p = 0; p < mPrograms.size(); ++p) {
	    output.append("Program ").append(p).append(": ").append(
		    mPrograms.get(p).execute()).append(NEW_LINE);
	    pause1();
	}
