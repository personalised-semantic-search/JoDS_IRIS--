...
public class WellFoundedSemanticsTest extends TestCase {
	...
	String program = "p('a') :- q('a'), ! r('a')." + "q('a') :- ! p('a')."
		+ "r('a').";
    ...
    public void testKempSrivastavaStuckeyExample2WithVariables()
	    throws Exception {
	String program = "p(?x) :- q(?x), ! r(?x)." + "q(?x) :- ! p(?x)."
		+ "r('a').";
	...
	String program = "p('a') :- q('a'), ! r('a')." + "q('a') :- ! q('a')."
		+ "r('a').";
    ...
    public void testKempSrivastavaStuckeyExample3WithVariables()
	    throws Exception {
	String program = "p(?x) :- q(?x), ! r(?x)." + "q(?x) :- ! q(?x)."
		+ "r('a').";
	...
	String program = "noise(?t) :- loaded(?t), shoots(?t)."
		+ "loaded(0)."
		+ "loaded(?t) :- succ( ?s, ?t ), loaded( ?s), not shoots( ?s )."
		+ "shoots( ?t ) :- triggers( ?t )." + "triggers(1)."
		+ "succ(0,1).";

	Helper.evaluateWellFounded(program + "?- loaded(?x).",
		"dummy(0).dummy(1).");
	...
	String program = "p(?x,?y) :- b(?x,?y)."
		+ "p(?x,?y) :- b(?x,?u), p(?u,?y)." + "e(?x,?y) :- g(?x,?y)."
		+ "e(?x,?y) :- g(?x,?u), e(?u,?y)." +

		"a(?x,?y) :- e(?x,?y), not p(?x,?y)." +

		"b(1,2).g(2,3)." + "b(2,1).g(3,2)." + "?- a(?x,?y).";

	Helper
		.evaluateWellFounded(program,
			"dummy(2,3).dummy(3,2).dummy(3,3).");
    }
}
