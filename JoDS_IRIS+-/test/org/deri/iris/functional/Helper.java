...
public class Helper {
     ...
     * and optimization. Assert that all evaluations produce the same, expected
     * results.
     * 
     * @throws Exception
     *             on failure
     */
    public static void evaluateWithAllStrategies(String program,
    ...
    public static void evaluateNotOptimised(String program,
	    String expectedResults) throws Exception {
    ...
    public static void evaluateNaive(String program, String expectedResults)
	    throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void evaluateSemiNaive(String program, String expectedResults)
	    throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void evaluateUnsafeRules(String program,
	    String expectedResults) throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
	...
	executeAndCheckResults(program, expectedResults, configuration,
		"Semi-Naive with unsafe-rules");
    }

    public static void evaluateWellFounded(String program,
	    String expectedResults) throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
	...
	executeAndCheckResults(program, expectedResults, configuration,
		"Well-founded semantics and unsafe rules");
    }
    ...
    public static void evaluateSLDNF(String program, String expectedResults)
	    throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void evaluateOLDT(String program, String expectedResults)
	    throws Exception {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
	...
	IKnowledgeBase kb = KnowledgeBaseFactory.createKnowledgeBase(parser
		.getFacts(), parser.getRules(), parser.getQueries(), configuration);
    ...
    private static void checkResults(String expected, IRelation actualResults)
	    throws Exception {
	if (PRINT_RESULTS)
	...
	if (expected == null || expected.trim().length() == 0) {
	    Assert.assertEquals("There are no results expected.", 0,
	    ...
	    IRelation expectedResults = expectedFacts.get(predicates.iterator()
		    .next());
    ...
    /**
     * Evaluate the given logic program with all evaluation strategies and
     * ensure that each fails with the expected exception.
     * 
     * @param program
     *            The logic program
     * @param expectedExceptionClass
     *            The exception class object expected or null for an unknown
     *            exception type.
     */
    public static void checkFailureWithAllStrategies(String program,
    ...
    public static void checkFailureWithAllSafeRulesOnly(String program,
	    Class<?> expectedExceptionClass) {
	checkFailureWithNaive(program, expectedExceptionClass);
	checkFailureWithSemiNaive(program, expectedExceptionClass);
    }

    public static void checkFailureWithNaive(String program,
	    Class<?> expectedExceptionClass) {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void checkFailureWithSemiNaive(String program,
	    Class<?> expectedExceptionClass) {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();

	checkFailure(program, expectedExceptionClass, configuration,
		"Semi-Naive");
    }

    public static void checkFailureWithUnSafeRules(String program,
	    Class<?> expectedExceptionClass) {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void checkFailureWithWellFounded(String program,
	    Class<?> expectedExceptionClass) {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
    ...
    public static void checkFailureWithOLDT(String program,
	    Class<?> expectedExceptionClass) {
	Configuration configuration = KnowledgeBaseFactory
		.getDefaultConfiguration();
     ...
     *            exception type.
     */
    public static void checkFailure(String knowledgeBase,
	    Class<?> expectedExceptionClass, Configuration configuration,
	    String evaluation) {
	try {
