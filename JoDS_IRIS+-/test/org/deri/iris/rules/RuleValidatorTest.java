...
public class RuleValidatorTest extends TestCase {
	...
	RuleValidator validator = validate("p(?x) :- q(?x), not r(?y).", false,
		true, false);
    ...
    public void testSafeWithComputedEqualityArgumentAndNoArithmeticImplications()
	    throws Exception {
	RuleValidator validator = validate("p(?y) :- q(?x), ?x = ?y.", true,
		false, false);
	...
	RuleValidator validator = validate("p(?y) :- q(?x), ?x + 1 = ?y.",
		true, false, false);
    ...
    public void testUnsafeWithNegatedComputedArithmeticArgument()
	    throws Exception {
	RuleValidator validator = validate("p(?y) :- q(?x), ! ?x + 1 = ?y.",
		true, false, false);
	...
	RuleValidator validator = new RuleValidator(rule, true, true, false);
	...
	assertTrue(validator.getAllUnlimitedVariables().contains(
		TERM.createVariable(variable)));
    }
    ...
    private static RuleValidator validate(String rule,
	    boolean allowNotLimitedVariablesInNegatedSubGoals,
	    boolean allowArithmeticPredicatesToImplyLimited,
	    boolean allowGuardedRules) throws ParserException {
	return new RuleValidator(parseRule(rule),
		allowNotLimitedVariablesInNegatedSubGoals,
		allowArithmeticPredicatesToImplyLimited, allowGuardedRules);
    }

    private static RuleValidator validate(String rule) throws ParserException {
	return new RuleValidator(parseRule(rule), true, true, false);
    }
