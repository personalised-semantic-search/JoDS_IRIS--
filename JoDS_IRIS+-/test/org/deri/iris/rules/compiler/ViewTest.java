...
public class ViewTest extends TestCase {
	...
	mRelation = new RelationFactory().createRelation();

	// f(1,2,g(2,3))
	ITuple t1 = Helper.createTuple(1, 2, Helper.createConstructedTerm("g",
		2, 3));
	mRelation.add(t1);

	// f(1,2,g(2,1))
	ITuple t2 = Helper.createTuple(1, 2, Helper.createConstructedTerm("g",
		2, 1));
	mRelation.add(t2);

	// f(X,Y,g(Y,X))
	ITuple viewCriteria = Helper.createTuple("X", "Y", Helper
		.createConstructedTerm("g", "Y", "X"));

	mView = new View(mRelation, viewCriteria, new RelationFactory());
    }
