...
public class CompiledRuleTest extends TestCase {
	...
	IFacts facts = new Facts(new RelationFactory());
	...
	bodyLiterals.add(Helper.createLiteral(true, "q", "X", Helper
		.createConstructedTerm("f", "Y")));
	bodyLiterals.add(Helper.createLiteral(true, "r", "Y", Helper
		.createConstructedTerm("h", "Z")));

	List<ILiteral> headLiterals = new ArrayList<ILiteral>();
	headLiterals.add(Helper.createLiteral(true, "p", Helper
		.createConstructedTerm("f", "X", "Y"), 2, "Z"));
	...
	for (int i = 0; i < count; ++i, ++v) {
	    Q.add(Helper.createTuple(v, Helper
		    .createConstructedTerm("f", v + 1)));
	    R.add(Helper.createTuple(v + 1, Helper
		    .createConstructedTerm("h", v)));
