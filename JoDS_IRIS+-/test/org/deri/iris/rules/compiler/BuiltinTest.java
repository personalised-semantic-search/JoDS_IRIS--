...
public class BuiltinTest extends TestCase {
	...
	IBuiltinAtom builtinPredicate = new AddBuiltin(criteria
		.toArray(new ITerm[0]));
	...
	IBuiltinAtom builtinPredicate = new AddBuiltin(builtinTuple
		.toArray(new ITerm[0]));
	...
	Builtin builtin = new Builtin(new ArrayList<IVariable>(),
		builtinPredicate, true, configuration);
    ...
    private static final IRelation mRelationWithOneZeroLengthTuple = new RelationFactory()
	    .createRelation();
    static {
